    ...
    for iSheet in range(1,3):
        ...
        for idx1, list1 in enumerate(paramNames[iSheet]):
            ...
            for idx2, cols2 in enumerate(list1):
                if cols2[0][-6:]=='Output': continue
                iTarget += 1
                print('Processed {}: Step #{}.'.format(dtype_lab,iTarget+1))
                # quit3()
                ...
                # took 9:00
                # Using the prior remainder variance, a depth-spanning 
                # weight function (DWF) was constructed (Goswami et al.,
                # 2014). This function evaluates an ensemble of depths 
                # for each calendar age, mapping an age to a collection 
                # of depths with probabilistic weights. For example, 
                # 1000 BP may correspond to 19.1 mm depth with 25% weight, 
                # 20.3 mm depth with 40% weight, and 25.2 mm depth with 
                # 35% weight.
                ...
                # Took 1:00
                # Using the DWF (depth-spanning weight function), we can
                # evaluate the posterior PDF of a proxy variable (e.g., 
                # trace element concentrations) at a given time using the 
                # proxy depth data. The PDF represents the relationship 
                # between trace element concentrations and age. For example, 
                # 1000 BP may correspond to a collection of trace element 
                # concentrations (e.g., 4.6 ppm with 20% weight, 4.81 ppm 
                # with 15% weight, 5.05 ppm with 35% weight, and 6.1 ppm 
                # with 30% weight).
