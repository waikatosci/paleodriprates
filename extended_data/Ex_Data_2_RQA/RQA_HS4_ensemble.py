...
from ngeo_style import (apply_style, style_ax, panel_label, SINGLE_COL, DOUBLE_COL,
                        COL_BG_900, COL_BG_700, COL_BG_600, COL_BG_400, COL_BG_300, COL_BG_200,
                        COL_TEAL_800, COL_TEAL_600, COL_TEAL_200, COL_BROWN_500, COL_BROWN_200,
                        COL_DORANGE, COL_RED_900, make_heatmap_cmap)
apply_style()
...
N_REALISATIONS = 200   # 2026-09-17: subsample of the 1,000-member ensemble (runtime); envelopes are 5-95% over these 200
...
def make_main_figure(plot_age, age_hi, age_lo,
    ...
    gs  = gridspec.GridSpec(4, 1, figure=fig, hspace=0.18,
                            left=0.10, right=0.88, top=0.97, bottom=0.08)
    ...
    for ax in (ax1, ax2, ax3, ax4):
        style_ax(ax)
        ax.tick_params(axis='both', which='major', right=True, direction='in')

    # Panel A: RQA d18O
    ax1.fill_between(plot_age, R(d18o_det_lo), R(d18o_det_hi),
                     color=COL_BG_400, alpha=0.35, lw=0, label='DET 5-95%ile')
    ax1.fill_between(plot_age, R(d18o_tran_lo), R(d18o_tran_hi),
                     color=COL_BROWN_200, alpha=0.7, lw=0, label='TRANS 5-95%ile')
    ax1.plot(plot_age, R(d18o_det_p),
             color=COL_BG_900, lw=0.7, label='Determinism (DET)')
    ax1.plot(plot_age, R(d18o_tran_p),
             color=COL_BROWN_500, lw=0.7, label='Transitivity (TRANS)')
    ax1.axhline(DET_THRESHOLD, color=COL_BG_600, lw=0.3, ls=':')
    ax1.set_ylabel(r'RQA ($\delta^{18}$O)', labelpad=8)
    ax1.set_ylim(0, 1)
    ax1.legend(loc='lower right', frameon=False, ncol=2, fontsize=5.5)
    panel_label(ax1, 'a')
    ...
    im2 = ax2.imshow(R(d18o_pdf).T,
                     ...
                     cmap=make_heatmap_cmap(), origin='lower', vmin=0, vmax=1)
    ax2.plot(plot_age, R(d18o_med_grid), color=COL_BG_900, lw=0.6, label='Median')
    ...
    ax3.plot(plot_age, R(drip_tran_p),
             color=COL_BROWN_500, lw=0.7, label='Transitivity (TRANS)')
    ax3.axhline(DET_THRESHOLD, color=COL_BG_600, lw=0.3, ls=':')
    ax3.set_ylabel('RQA (drip rate)', labelpad=8)
    ax3.set_ylim(0, 1)
    ax3.legend(loc='lower right', frameon=False, ncol=2, fontsize=5.5)
    panel_label(ax3, 'c')
    ...
    im4 = ax4.imshow(R(drip_pdf).T,
                     ...
                     cmap=make_heatmap_cmap(), origin='lower', vmin=0, vmax=1)
    ax4.plot(plot_age, R(drip_med_grid), color=COL_BG_900, lw=0.6, label='Median')
    ax4.plot(plot_age, R(drip_pc25_grid), color=COL_BG_900, ls='--', lw=0.3)
    ax4.plot(plot_age, R(drip_pc75_grid), color=COL_BG_900, ls='--', lw=0.3)
    ...
    # Shared colorbar
    cbar_ax = fig.add_axes([0.905, 0.08, 0.010, 0.89])
    cbar    = fig.colorbar(im4, cax=cbar_ax, ticks=[0, 0.5, 1])
    cbar.set_label('Normalised PDF', fontsize=5.5, labelpad=3)
