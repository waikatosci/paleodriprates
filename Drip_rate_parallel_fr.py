...
baseRow = 19
StationName = wsheets[3].cell(baseRow,2).value
if StationName is None:
    print("Station Name is required!")
    exit()
StationNameShort = wsheets[3].cell(baseRow+1,2).value
if StationNameShort is None:
    print("Short Station Name is required!")
    exit()
...
bProxyRecord = True if wsheets[3].cell(baseRow+2,2).value in ['Yes','True',1] else False
CALAGE_MIN = wsheets[3].cell(baseRow+3,2).value
if CALAGE_MIN is None or type(CALAGE_MIN)!=int:
    print("Calibration Age Minimum (BP, integer) is required!")
    exit()
CALAGE_MAX = int(wsheets[3].cell(baseRow+4,2).value)
if CALAGE_MAX is None or type(CALAGE_MAX)!=int:
    print("Calibration Age Maximum (BP, integer) is required!")
    exit()
...
if bProxyRecord:
    ...
    print('Start a long proxy record processing. This will take more than 18 minutes')
    ...
    # Import data from 1.Depth_Age, 2.Trace_Elems, 3.Isotopes sheets
    # iSheet = 2
    bError = False
    ...
    # Setup a model relating age with depth
    ...
    # Age, trace elements, and isotopes of the stalagmite were measured 
    # at different depths. To evaluate these data at common depths, we 
    # compiled the depths associated with each measurement and integrated 
    # them into a unified depth profile.
    ...
    # Adjusts the prior remainder variance of the age errors such that the
    # standard deviation and mean of the posteriors are as close to the
    # standard deviation and mean of the age measurements, 
    # using the Moving Taylor Bayesian regression model to predict age as 
    # a function of unified depth with a maximum order of 2 (Heitzig, 2013)
    ...
    # Scale the prior remainder variance
    ...
    # Multiprocessing is needed but was not implemented
    # set BayProx:Data:SampleInfo
    bError = False
    pLst = []
    iTarget = -1
    # Process the data from 1.Depth_Age and 2.Trace_Elems,
    # iSheet = 1
    for iSheet in range(1,3):
        # break
        dtype_lab = ['age','concentration','isotope'][iSheet]
        ...
        for idx1, list1 in enumerate(paramNames[iSheet]):
            ...
            for idx2, cols2 in enumerate(list1):
                if cols2[0][-6:]=='Output': continue
                iTarget += 1
                print('Processing {}: Step #{}.'.format(dtype_lab,iTarget+1))
                # quit3()
                ...
                # Setup a depth-spanning weight function model
                # set BayProx:AgeDepth:DWF
                # DWF.output contains all the relevant info
                # print("Estimating DWFs ...")
                # Fix MoTaBaR parameters
                ...
                DWF = ad.DWF(DT1)
                # Parallel Start
                # Collect task information and run later in parallel
                tinfo = dict(iTarget=iTarget, PD=PD, calage=calage,
                                max_degree=max_degree, pre_remvar=pre_remvar,
                                method=method, iSheet=iSheet,
                                idx1=idx1, idx2=idx2, name2=name2)
                pLst.append(tinfo)
    ...
    # Run collected proxy tasks in parallel threads and then persist results
    def process_proxy_task(tinfo):
        """Worker function to compute DWF, PDist, and proxy estimates.

        This runs in a thread and returns a serializable dict with results.
        """
        iTarget = tinfo['iTarget']
        PD = tinfo['PD']
        calage = tinfo['calage']
        max_degree = tinfo['max_degree']
        pre_remvar = tinfo['pre_remvar']
        method = tinfo['method']
        iSheet = tinfo['iSheet']
        name2 = tinfo['name2']
        ...
        # ensure PEst.mean_value exists for compatibility with old code
        try:
            mean_value = PEst.mean_value
        except Exception:
            mean_value = prx_mean
        ...
        return dict(iTarget=iTarget, name2=name2, iSheet=iSheet,
            prx_mean=prx_mean, prx_std=prx_std,
            prx_pc05=prx_pc05, prx_pc10=prx_pc10,
            prx_pc25=prx_pc25, prx_pc50=prx_pc50,
            prx_pc75=prx_pc75, prx_pc90=prx_pc90,
            prx_pc95=prx_pc95,
            PDist=PDist, PEst=PEst,
            idx1=tinfo['idx1'], idx2=tinfo['idx2'])

    # execute tasks in parallel using a ThreadPoolExecutor so we avoid
    # possible pickling issues with complex BayProx objects
    results = []
    max_workers = min(4, (os.cpu_count() or 1))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = [ex.submit(process_proxy_task, t) for t in pLst]
        for fut in concurrent.futures.as_completed(futures):
            results.append(fut.result())

    # Write results into Excel and update dataAll
    baseRow = 5
    baseCol = 10
    for res in sorted(results, key=lambda x: x['iTarget']):
        iTarget = res['iTarget']
        name2 = res['name2']
        colName = ['calage','mean','std','pc50','pc05','pc10','pc25','pc75','pc90','pc95']
        colVars = [calage, res['prx_mean'], res['prx_std'], res['prx_pc50'], res['prx_pc05'], res['prx_pc10'], res['prx_pc25'], res['prx_pc75'], res['prx_pc90'], res['prx_pc95']]
        colLen = len(colName)
        ...
        # update dataAll with full PDist and PEst deep-copies
        idx1 = res['idx1']
        idx2 = res['idx2']
        dataAll[res['iSheet']][idx1][idx2][3] = copy.deepcopy(res['PDist'])
        dataAll[res['iSheet']][idx1][idx2][4] = copy.deepcopy(res['PEst'])
...
TE1 = {'elem':wsheets[1].cell(6,4).value, 'mol_wt':wsheets[1].cell(6,7).value, 
       ...
       'F':wsheets[1].cell(6,10).value,'InertF':wsheets[1].cell(6,11).value,
...
TE2 = {'elem':wsheets[1].cell(8,4).value, 'mol_wt':wsheets[1].cell(8,7).value, 
       ...
       'F':wsheets[1].cell(8,10).value,'InertF':wsheets[1].cell(8,11).value,
       'aq_conc':np.float64(wsheets[1].cell(8,9).value), 
       'ca_conc':np.float64(wsheets[1].cell(11,2).value),
       'PDist':copy.deepcopy(dataAll[1][1][0][3])}

# Once the time t is given, we can estimate the ensemble of depths with weights, 
# which in turn determines the ensemble of trace element concentrations with 
# weights. We evaluated the probability of the drip rate v from 0.02 to 100 per 
# minute at intervals of 0.02 and constructed the PDF by combining these 
# evaluations. These steps were repeated from -50 BP to 9500 BP at 1-year 
# interval.

# Load the probability distribution of driprate (drips/minute) vs. time
# using the first trace element
# V_span is the span of the drip rates (0.02, 0.04, ... 100)
...
N_realisations = 1000          # adjust as needed
rng = np.random.default_rng(seed=42)

realisations = np.zeros((N_realisations, len(V_age)))

for i in range(V_pdf.shape[1]):
    cdf = np.cumsum(V_rsw * V_pdf[:, i])
    cdf /= cdf[-1]                            # ensure it reaches exactly 1
    u = rng.uniform(0., 1., N_realisations)
    f_ = interp1d(cdf, V_span, kind="linear",
                  bounds_error=False, fill_value=(V_span[0], V_span[-1]))
    realisations[:, i] = f_(u)

out = np.vstack([V_age, realisations])
header = "age," + ",".join([f"r{j}" for j in range(N_realisations)])
np.savetxt("drip_rate_realisations.csv", out.T,
           delimiter=",", header=header, comments="")
print(f"Saved {N_realisations} realisations to drip_rate_realisations.csv")
# ────────────────────────────────────────────────────────────────────────

# ── Sample realisations from the d18O PDist for RQA ─────────────────────
# dataAll[2] holds the BayProx isotope results, processed in exactly the
# same pipeline as the trace elements above.  Index [0][0][3] is the PDist
# object for the first isotope series (d18O); [1][0][3] would be d13C.
# We sample from PDist.pdfmat using the same inverse-CDF method used above
# for drip rate, so both realisation files are methodologically symmetric.
#
# PDist.pdfmat  : shape (n_proxy_values, n_ages)  — normalised PDF columns
# PDist.proxyspan : 1-D array of proxy values corresponding to rows of pdfmat
#
# The age axis for isotopes is calage (same grid used throughout BayProx),
# extracted via the DWF/PDist pipeline, so we reuse V_age as the time axis.
# -------------------------------------------------------------------------
iso_idx = plotConfs[1]['Plot2Isotope'] - 1   # 0 = d18O, 1 = d13C (from settings)
PDist_iso = copy.deepcopy(dataAll[2][iso_idx][0][3])

iso_span = PDist_iso.proxyspan          # proxy value axis (e.g. d18O in ‰)
iso_pdf  = PDist_iso.pdfmat             # shape: (n_vals, n_ages)

# Build half-widths for numerical integration (same as V_rsw above)
iso_rsw = 0.5 * np.r_[
    iso_span[1]  - iso_span[0],
    iso_span[2:] - iso_span[:-2],
    iso_span[-1] - iso_span[-2]
]

iso_realisations = np.zeros((N_realisations, iso_pdf.shape[1]))

for i in range(iso_pdf.shape[1]):
    col   = iso_pdf[:, i]
    cdf   = np.cumsum(iso_rsw * col)
    cdf  /= cdf[-1]                      # normalise to [0, 1]
    u     = rng.uniform(0., 1., N_realisations)
    f_iso = interp1d(cdf, iso_span, kind="linear",
                     bounds_error=False,
                     fill_value=(iso_span[0], iso_span[-1]))
    iso_realisations[:, i] = f_iso(u)

iso_out    = np.vstack([V_age, iso_realisations])
iso_header = "age," + ",".join([f"r{j}" for j in range(N_realisations)])
np.savetxt("d18O_realisations.csv", iso_out.T,
           delimiter=",", header=iso_header, comments="")
print(f"Saved {N_realisations} d18O realisations to d18O_realisations.csv")
# ────────────────────────────────────────────────────────────────────────
