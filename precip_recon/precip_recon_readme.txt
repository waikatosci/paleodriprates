...
5. **T_recon_Wang_et_al.xlsx**
   - **Description:** Excel file containing age-temperature data from Wang et al. QSR 2018
   - **Sheets:**
     - "Sheet1": Ages BP, RAN15-MAAT temperatures (°C) from Wang et al. (2018) 
...
2. **Review Input Data:**
   ...
   - Check `T_recon_Wang_et_al.xlsx` for temperature proxy (warmer mid-Holocene).
...
3. **Run the Notebook (`P_quantification_Holocene.ipynb`):**
   ...
   - **Key Steps in Code:**
     - Load data from `T_recon_Wang_et_al.xlsx` and `drip_rate_percentiles.xlsx`.
